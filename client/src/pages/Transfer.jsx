...
export default function Transfer() {
  ...
  const recipientPk =
    peer?.wallets?.find((w) => w.is_primary)?.public_address || peer?.wallets?.[0]?.public_address;
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col bg-surface">
      <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-white/5 bg-surface/90 px-3 py-3 backdrop-blur">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-slate-500">
            {recipientPk ? `${recipientPk.slice(0, 4)}…${recipientPk.slice(-4)}` : 'No pubkey'}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-3 py-4 pb-36">
        {loadErr && <p className="text-center text-sm text-red-400">{loadErr}</p>}
        {timeline.map((item) => {
          if (item.kind === 'transfer') {
            const tr = item.transfer;
            const mine = tr.sender_id === user?.id;
            return (
              <div key={`t-${tr.id}`} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div
                ...
                >
                  ...
                  {tr.tx_hash && (
                    <p className="mt-1 truncate text-[10px] opacity-70">{tr.tx_hash}</p>
                  )}
                  <p className="mt-1 text-[10px] opacity-60">{formatTime(tr.createdAt)}</p>
                </div>
              </div>
            );
          }
          ...
          return (
            <div key={`m-${msg.id}`} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div
              ...
              >
                <p>{msg.text}</p>
                <p className="mt-1 text-[10px] text-slate-500">{formatTime(msg.createdAt)}</p>
              </div>
            </div>
          );
        })}
      ...
      <footer className="fixed bottom-0 left-0 right-0 border-t border-white/10 bg-surface/95 p-3 backdrop-blur">
        <div className="mx-auto flex max-w-md gap-2">
          ...
          <button
            type="button"
            onClick={() => setSheet(true)}
            className="rounded-full bg-accent2 p-3 text-white"
          >
